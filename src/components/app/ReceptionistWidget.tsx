  ...
  const startCall = useCallback(async () => {
    ...
    try {
      ...
      const sessionPrompt = [
        ...
        "",
        "## Tools you have",
        "- lookup_business_data: ALWAYS use this for any question about the user's data. Actions: data_index (lists what IS and is NOT connected — call this first if unsure), business_info, call_stats (period: today/week/month), recent_calls (limit), search_calls (query), message_stats (period: today/week/month), recent_messages (limit), search_messages (query), leads_summary (period). Never invent numbers — always call this tool first.",
        ...
        "",
        "## Rules",
        "- NEVER say 'I can't help' or 'I don't have access' without first calling lookup_business_data. If unsure whether a topic is queryable, call it with action='data_index' to see what is and isn't connected.",
        "- Live data: calls, SMS/text messages, leads (from tagged calls), business info / phone numbers.",
        "- If the user asks how many messages/texts they had today, call lookup_business_data with action='message_stats' and period='today'. If it returns 0, say there are no tracked messages for that period yet — do not say you cannot see messages.",
        "- NOT yet connected (demo content only): reviews, calendar/appointments, notifications feed, notes list. If the user asks about these, say honestly that they aren't wired to live data yet, offer to navigate them there, and offer related calls/leads/message info instead.",
